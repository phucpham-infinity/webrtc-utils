...
function insertScriptSrcInHtmlDom(scriptSrc: string) {
  //Create a script tag
...
function noop(error: any) {
  if (error) logger.error(error);
}

function trackStop(track: { stop: () => any }) {
  track.stop && track.stop();
}

function streamStop(stream: { getTracks: () => any[] }) {
 ...
 */
var dumpSDP = function (description: { type: string; sdp: string } | null) {
...
function bufferizeCandidates(
  pc: {
    signalingState: string;
    addIceCandidate: (arg0: any, arg1: any, arg2: any) => void;
  ...
  function setSignalingstatechangeAccordingWwebBrowser(
  ...
  return function (candidate: any, callback: (arg0: Error) => void) {
...
function removeFIDFromOffer(sdp: string | string[]) {
...
function setIceCandidateAccordingWebBrowser(
  functionToExecute: (event: any) => void,
  pc: {
    onicecandidate: any;
    addEventListener: (arg0: string, arg1: any) => void;
  }
...
function WebRtcPeer(
  ...
  options:
    ...
    | undefined,
  callback: ((arg0: string | undefined) => void) | null | undefined
) {
  ...
  if (!pc.getLocalStreams && pc.getSenders) {
    pc.getLocalStreams = function () {
      ...
      pc.getSenders().forEach(function (sender: { track: MediaStreamTrack }) {
  ...
  if (!pc.getRemoteStreams && pc.getReceivers) {
    pc.getRemoteStreams = function () {
      var stream = new MediaStream();
      pc.getReceivers().forEach(function (sender: { track: MediaStreamTrack }) {
        stream.addTrack(sender.track);
      });
      return [stream];
    };
  ...
  var iceCandidateFunction = function (event: { candidate: any }) {
  ...
  this.on(
    "newListener",
    function (event: string, listener: (arg0: undefined) => void) {
    ...
    if (
      ...
      var setLocalDescriptionOnSuccess = function () {
        ...
        if (multistream && usePlanB) {
          ...
          logger.debug(
            "offer::origPlanB->UnifiedPlan",
            dumpSDP(localDescription)
          );
        }
        callback(null, localDescription.sdp, self.processAnswer.bind(self));
      };
      var createOfferOnSuccess = function (offer: any) {
  ...
  function mangleSdpToAddSimulcast(answer: RTCSessionDescription) {
    if (simulcast) {
      if (browser.name === "Chrome" || browser.name === "Chromium") {
  ...
  function start() {
    ...
    if (audioStream) {
      audioStream.getTracks().forEach(function (track: any) {
  ...
  if (mode !== "recvonly" && !videoStream && !audioStream) {
    function getMedia(constraints: MediaStreamConstraints | undefined) {
    ...
    if (sendSource === "webcam") {
      ...
      getScreenConstraints(
        sendSource,
        function (error: any, constraints_: any) {
...
function createEnableDescriptor(type: string) {
  ...
  return {
    ...
    get: function () {
      ...
      for (
        var i = 0, stream: { [x: string]: () => any };
        (stream = streams[i]);
        i++
      ) {
        var tracks = stream[method]();
        for (var j = 0, track: { enabled: any }; (track = tracks[j]); j++)
    ...
    set: function (value: any) {
      function trackSetEnable(track: { enabled: any }) {
        track.enabled = value;
      }

      this.peerConnection
        .getLocalStreams()
        .forEach(function (stream: { [x: string]: () => any[] }) {
...
WebRtcPeer.prototype.getLocalStream = function (index: any) {
  if (this.peerConnection) {
    return this.peerConnection.getLocalStreams()[index || 0];
  }
};

WebRtcPeer.prototype.getRemoteStream = function (index: any) {
  if (this.peerConnection) {
...
function WebRtcPeerRecvonly(options: any, callback: any) {
...
function WebRtcPeerSendonly(options: any, callback: any) {
...
function WebRtcPeerSendrecv(options: any, callback: any) {
...
function harkUtils(stream: any, options: any) {
